/**
 * Manager Management Feature
 * Admin Dashboard child page for creating and managing Manager accounts.
 */
...
export default function StaffManagement({ token }) {
    ...
    const fetchStaff = async () => {
        try {
            ...
            if (res.ok) {
                ...
                setStaff(data.filter(u => ['manager', 'hr'].includes(u.role)));
            }
        } catch (err) {
            console.error(err);
        }
    ...
    const handleCreate = async (e) => {
        ...
        try {
            const res = await fetch('/api/v1/admin/create-manager', {
                method: 'POST',
                headers: {
            ...
            setFormData({ name: '', email: '', password: '', department: '', role: 'manager' });
    ...
    return (
        <div style={{ padding: 24, maxWidth: 1200, margin: '0 auto', color: '#e2e8f0', fontFamily: 'Inter, sans-serif' }}>
            <h2 style={{ fontSize: 28, marginBottom: 8, fontWeight: 700 }}>👥 Staff Management</h2>
            <p style={{ color: '#94a3b8', marginBottom: 32 }}>Create and manage dashboard accounts (Managers & HR).</p>
            ...
            <div style={{ display: 'flex', gap: 24, flexWrap: 'wrap', alignItems: 'flex-start' }}>
                ...
                <div style={{
                    ...
                    <h3 style={{ marginTop: 0, marginBottom: 20, color: '#f8fafc' }}>Create New Staff</h3>
                    ...
                    <form onSubmit={handleCreate}>
                        ...
                        <label style={{ fontSize: 12, color: '#94a3b8', marginBottom: 4, display: 'block' }}>Role</label>
                        ...
                        <input type="text" name="password" value={formData.password} onChange={handleChange} placeholder="Min 8 characters" style={inputStyle} />
                        ...
                        <button type="submit" style={{
                            ...
                            marginTop: 8, boxShadow: '0 4px 12px rgba(59, 130, 246, 0.3)'
                        }}>
                            + Create Staff
                        </button>
                    </form>
                ...
                <div style={{
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
                        <h3 style={{ margin: 0, color: '#f8fafc' }}>Active Staff ({staff.filter(m => m.is_active).length})</h3>
                        <button onClick={fetchStaff} style={{ background: 'transparent', border: '1px solid #475569', color: '#cbd5e1', padding: '6px 12px', borderRadius: 6, cursor: 'pointer', fontSize: 13 }}>
                            🔄 Refresh
                        </button>
                    </div>
                            ...
                            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 14 }}>
                                <thead>
                                    <tr style={{ borderBottom: '1px solid #334155', color: '#94a3b8', textAlign: 'left' }}>
                                        ...
                                        <th style={{ padding: '12px 0', fontWeight: 600 }}>Role</th>
                                ...
                                <tbody>
                                    {staff.length === 0 ? (
                                        <tr><td colSpan="6" style={{ textAlign: 'center', padding: 32, color: '#64748b' }}>No staff found.</td></tr>
                                    ) : staff.map(m => (
                                        <tr key={m.id} style={{ borderBottom: '1px solid #334155' }}>
                                            <td style={{ padding: '14px 0', fontWeight: 500 }}>{m.username}</td>
                                            <td style={{ padding: '14px 0', color: '#cbd5e1' }}>{m.email}</td>
                                            <td style={{ padding: '14px 0' }}>
                                                <span style={{
                                                    background: m.role === 'manager' ? '#065f46' : '#5b21b6', 
                                                    color: m.role === 'manager' ? '#34d399' : '#c084fc', 
                                            ...
                                            <td style={{ padding: '14px 0' }}>
                                                {m.is_active ? 
                                                    ...
                                                    <span style={{ color: '#ef4444', fontSize: 12 }}>● Inactive</span>
                                                }
                                            </td>
                                            <td style={{ padding: '14px 0', textAlign: 'right' }}>
                                                {m.is_active && (
                                                    <button 
                                                        onClick={() => handleDeactivate(m.id, m.username)}
